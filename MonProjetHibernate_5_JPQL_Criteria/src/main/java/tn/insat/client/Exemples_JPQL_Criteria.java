...
public class Exemples_JPQL_Criteria {
	...
	private static void example_1_select_wrong_class() {
		try {
			...
			Query query = em.createQuery("SELECT c FROM  TTTT as c");
	...
	private static void example_11_StoredProcedure() {
		...
		try {
			...
			spq.setParameter("num", "C1000");
			System.out.println(spq.getSingleResult());
	...
	private static void example_8_Criteria_list_of_objects() {
		try {
			...
			cq.where(cb.greaterThan(e.get("solde"), 3000));
			cq.select(e);
	...
	private static void example_9_Criteria_single_object() {
		try {
			...
			CriteriaQuery cq = cb.createQuery();
			Root e = cq.from(Compte.class);
			cq.where(cb.equal(e.get("numero"),
					cb.parameter(String.class, "id")));
            cq.select(e);
			Query query = em.createQuery(cq);


			query.setParameter("id", "C1000");
